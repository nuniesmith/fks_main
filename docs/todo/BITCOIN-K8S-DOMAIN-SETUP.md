# Bitcoin Signal Demo - Kubernetes Domain Setup (fkstrading.xyz)
...
```bash
cd repo/main
./scripts/deploy-fkstrading-domain.sh
```
...
helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx \
  ...
  --set controller.service.externalIPs[0]=100.80.141.117 \
  --wait \
  --timeout 5m
```
...
### Complete Workflow
1. **Deploy Platform**: Run `./scripts/deploy-fkstrading-domain.sh`
...
**Next Action**: Run `./scripts/deploy-fkstrading-domain.sh` and then `minikube tunnel`!
